...
TEST(BitboardUtils, GetBit) {
    EXPECT_FALSE( GetBit(ZERO, 0) );
    EXPECT_FALSE( GetBit(ZERO, 50) );
    EXPECT_FALSE( GetBit(0x9102289248220842, 16) );
    EXPECT_TRUE( GetBit(0x9102289248220842, 63) );
}
TEST(BitboardUtils, BitscanForward) {
    EXPECT_EQ(BitscanForward(ZERO), -1); //expected behaviour for empty bitboards
    EXPECT_EQ(BitscanForward((u64)0x4000000000010), 4); //Bit index: 4 and 50
    EXPECT_EQ(BitscanForward((u64)0x4000000000000), 50); //Bit index: 50
}
TEST(BitboardUtils, BitscanReverse) {
    EXPECT_EQ(BitscanReverse(ZERO), -1);
    EXPECT_EQ(BitscanReverse((u64)0x4000000000010), 50); //Bit index: 4 and 50
    EXPECT_EQ(BitscanReverse((u64)0x10), 4); //Bit index: 4
}
