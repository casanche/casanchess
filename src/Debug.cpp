...
bool BoardIntegrityChecker::CheckIntegrity(const Board& board) {
    ...
    for (COLOR c : {WHITE, BLACK}) {
        for (PIECE_TYPE pt = PAWN; pt <= KING; ++pt) {
            const Bitboard piece_bb = board.m_pieces[c][pt];
            assert((allPiecesFromLists & piece_bb) == ZERO); // Check that the pieces do not overlap
            allPiecesFromLists |= piece_bb;
        }
    }
    bitboardIntegrity &= (allPiecesFromLists == board.m_allpieces); // m_allpieces should match piece-lists
 
    // Chess rules consistency
    return bitboardIntegrity &&
        (board.m_pieces[WHITE][PAWN] & (MaskRank[RANK1] | MaskRank[RANK8])) == 0 &&
        (board.m_pieces[BLACK][PAWN] & (MaskRank[RANK1] | MaskRank[RANK8])) == 0 &&
